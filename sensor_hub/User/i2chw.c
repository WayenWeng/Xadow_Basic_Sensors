...
void I2C_Call_Back(void)
{
    ...
    I2CFlagStatus = (unsigned long)(I2C1->ISR & (uint16_t)0x0000100FE);
		...
		else if((I2CFlagStatus & I2C_ISR_TXIS) != 0)
		{
				...
				{
						case 0: // scan
								I2C1->TXDR = sensor_scan[i2c_byte_count];
								i2c_byte_count ++;
								if(i2c_byte_count >= 4)
								{
										i2c_byte_count = 0;
						...
						case 36: // temp
								if(i2c_byte_count < 4)
								{
										I2C1->TXDR = sensor_id_type[(i2c_comand - 4) >> 3][i2c_byte_count];	
								}
								else 
								{
										I2C1->TXDR = sensor_data[(i2c_comand - 4) >> 3][i2c_byte_count - 4]; 
								}									
								i2c_byte_count++; 
								if(i2c_byte_count >= 8)
								{            
		...
		else if((I2CFlagStatus & I2C_ISR_RXNE) != 0)
		{
				...
				i2c_comand = (unsigned char)I2C1->RXDR;		
				i2c_byte_count = 0;
			
				if(i2c_comand == GPIO_TEST_CMD)
				sys_gpio_test = 1;
		}
		// If NACK event
